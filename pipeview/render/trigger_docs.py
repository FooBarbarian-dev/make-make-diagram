...
def strip_provenance(text: str) -> str:
    """Mask everything regeneration legitimately changes without the docs'
    content changing: the marker comment (commit, versions, hashes), the
    visible "Generated by pipeview" line, and the index's regeneration
    command (whose flags depend on how generation was invoked)."""
    text = re.sub(r"<!-- " + re.escape(MARKER) + r":.*?-->",
                  "<!-- provenance -->", text, flags=re.DOTALL)
    text = re.sub(r"^\*Generated by pipeview from .*$", "*provenance*",
                  text, flags=re.MULTILINE)
    text = re.sub(r"(Regenerate these docs.*?\n\n```\n).*?(\n```)",
                  r"\1<command>\2", text, flags=re.DOTALL)
    return text


def compare_docs_folder(dirpath: str, files: dict[str, str]) -> dict:
    """Drift check for `scenarios verify`: committed docs in ``dirpath``
    against freshly generated ``files``, provenance masked. Read-only.

    Returns {'ok', 'missing', 'stale', 'orphaned'} lists of filenames —
    orphaned = a marker-bearing file no current scenario produces. Files
    without the marker are human docs and are ignored (an expected name
    held by a marker-less file counts as missing: whatever it is, it is
    not the generated doc)."""
    ...
    if os.path.isdir(dirpath):
        for name in sorted(os.listdir(dirpath)):
            ...
            if MARKER in text[:4096]:
                on_disk[name] = text
    result: dict[str, list[str]] = {"ok": [], "missing": [], "stale": [],
                                    "orphaned": sorted(set(on_disk) - set(files))}
    for name in sorted(files):
        if name not in on_disk:
            result["missing"].append(name)
        elif strip_provenance(on_disk[name]) != strip_provenance(files[name]):
